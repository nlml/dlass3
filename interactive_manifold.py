...
def interactive_manifold(manifold, indices, cifar10, mean_image, title, full_title):
    num_classes = 10
    ...
    savedir = 'tmp_imgs_' + title + '/' # WILL BE DELTED FIRST!!!
    if exists(savedir):
        rmtree(savedir)
        mkdir(savedir)
    else:
    ...
    # Aaand plot it
    output_file("manifold" + title + ".html")
    ...
    p = figure(plot_width=1400, plot_height=800, tools=[hover],
               title=full_title + " (mouse over the dots to see images)")
...
if __name__ == '__main__':
    ...
    interactive_manifold(manifold, indices, cifar10, mean_image)